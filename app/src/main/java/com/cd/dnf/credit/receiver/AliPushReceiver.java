...
import com.cd.dnf.credit.bean.NotificationBean;
...
public class AliPushReceiver extends MessageReceiver {
    ...
    public void onNotification(Context context, String title, String summary, Map<String, String> extraMap) {
        ...
        String jsonStr= JSON.toJSONString(extraMap);
        handlePushSource(context,jsonStr);
        Log.i(REC_TAG,"收到一条推送通知 ： " + title + ", summary:" + summary);

    }

    private void handlePushSource(Context context,String jsonStr){
        AppPreferences mAppPreferences=new AppPreferences(context);
        String userId=mAppPreferences.get(AppPreferences.USER_ID, "");
        if(!TextUtils.isEmpty(userId)){
            NotificationBean notificationBean=JSON.parseObject(jsonStr,NotificationBean.class);
            if(TextUtils.equals(CreditUtil.NOTIFICATION_JPUSH_ORDER_TYPE,notificationBean.getType())){
                //推送订单状态改变
                CreditNotification notification = new CreditNotification();
                notification.setNotifyType(CreditNotifyType.NOTIFY_JPUSH_ORDER_STATUS_CHANGE);
                notification.setNotifyData("");
                EventBus.getDefault().post(notification);
            }else {
                CreditNotification notification = new CreditNotification();
                notification.setNotifyType(CreditNotifyType.NOTIFY_JPUSH_CHANGE);
                notification.setNotifyData("");
                EventBus.getDefault().post(notification);
            }
        }
    ...
    public void onMessage(Context context, CPushMessage cPushMessage) {
        ...
        handlePushSource(context,cPushMessage.getContent());
    }
