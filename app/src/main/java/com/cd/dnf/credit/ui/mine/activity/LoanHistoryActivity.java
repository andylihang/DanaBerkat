...
import com.cd.dnf.credit.ui.mine.adapter.LoanHistoryAdapter;
...
public class LoanHistoryActivity extends CreditSwipBackActivity implements BGARefreshLayout.BGARefreshLayoutDelegate{
    ...
    private List<LoanHistoryBean> mDataSource=new ArrayList<>();
    private FirebaseAnalytics mFirebaseAnalytics;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_loan_history_layout);
        ...
        mFirebaseAnalytics.logEvent(AnalyticUtil.REPAY_HISTORY_PAGE,null);
    }

    private void setupRecycleview() {
        mAdapter = new LoanHistoryAdapter(this);
    ...
    private void fetchLoanHistory(final int page){
        ...
        OkGo.<String>post(CreditApi.API_REPAYMENT_HISTORY).upJson(jsonStr).tag("fetchLoanHistory").execute(
                new CreditApiRequstCallback(this, false) {
                    ...
                    public void RequestSuccess(String body) {
                        ...
                        if (page == 1) {
                            currentPage = 1;
                            mDataSource = JSON.parseArray(pageBean.getData(), LoanHistoryBean.class);
                            mRefreshLayout.endRefreshing();
                            handleNoDataSource();
                        } else {
                            currentPage++;
                            mDataSource.addAll(JSON.parseArray(pageBean.getData(), LoanHistoryBean.class));
                            mRefreshLayout.endLoadingMore();
                        }
    ...
    public boolean onBGARefreshLayoutBeginLoadingMore(BGARefreshLayout refreshLayout) {
        if (mDataSource.size() < totalCout) {
            fetchLoanHistory(currentPage + 1);
            return true;
        }
        return false;
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        //取消网络请求
        OkGo.getInstance().cancelTag("fetchLoanHistory");
    }
